...
| `Could not locate the bindings file` (better-sqlite3) | Almost impossible after v13: the package ships N-API prebuilds for linux/darwin/win32 in its tarball, and a root `overrides` entry dedupes everything to that single v13 copy. If bindings still go missing (e.g. a tool pruned `prebuilds/`), re-run `npm install` or `npm ci`. |
