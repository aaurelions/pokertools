...
export function handleTimeout(state: GameState, action: TimeoutAction): GameState {
  ...
  // Use the normal betting handlers so a timeout fold can award the last live hand.
  const next = needsToCall
    ? handleFold(state, {
  ...
  newPlayers[seat] = { ...newPlayers[seat]!, isSittingOut: true };
  const history = [...next.actionHistory];
  history[state.actionHistory.length] = { ...history[state.actionHistory.length], action };
  return { ...next, players: newPlayers, actionHistory: history, timeBankActiveSeat: null };
}
