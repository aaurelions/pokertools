-- =============================================================================
-- 001_initial_schema.sql — PokerTools PostgreSQL production schema
--
-- Mirrors the existing Prisma / SQLite schema.  Idempotent: every statement
-- uses IF NOT EXISTS / IF EXISTS so the migration can be re-run safely.
--
-- Includes Tournament.lastBlindAdvancedAt for automatic MTT blind progression.
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- 2. Tables with foreign keys to already-created tables
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Tournament / Table — circular FK dependency resolved via deferred
--    constraint addition.  Table.tournamentId is nullable, so create Table
--    without the FK first, then Tournament (references Table via tableId),
--    then add the FK back on Table.
-- ---------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS "Table" (
    ...
    "state"        JSONB,
    "tournamentId" TEXT,                                           -- FK added below after Tournament exists
    "createdAt"    TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
    "updatedAt"    TIMESTAMPTZ   NOT NULL
);
...
CREATE TABLE IF NOT EXISTS "Tournament" (
    ...
    "lastBlindAdvancedAt" TIMESTAMPTZ,
...
-- Add the FK on Table.tournamentId → Tournament now that Tournament exists.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'Table_tournamentId_fkey'
    ) THEN
        ALTER TABLE "Table"
            ADD CONSTRAINT "Table_tournamentId_fkey"
...
-- ---------------------------------------------------------------------------
-- 4. Remaining tables (depend on Tournament and/or Table)
-- ---------------------------------------------------------------------------
